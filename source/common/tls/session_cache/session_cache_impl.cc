...
void GrpcClientImpl::storeTlsSessionCache(Network::TransportSocketCallbacks* callbacks, SSL* ssl, int index, const std::string& session_id, const uint8_t* session_data, std::size_t size) {
  ASSERT(ssl != nullptr);
  ...
  ENVOY_LOG(debug, "Sending request to store session_id: {} and session_key: {}", session_id, *session_data);
...
void GrpcClientImpl::fetchTlsSessionCache(Network::TransportSocketCallbacks* callbacks, SSL* ssl, int index, const std::string& session_id, uint8_t* session_data, std::size_t* len) {
  ASSERT(ssl != nullptr);
  callbacks_ = callbacks;
  ssl_ = ssl;
  index_ = index;
...
void GrpcClientImpl::onSuccess(
    ...
    ENVOY_LOG(debug, "response -  type: {}  code: {} session ID: {}  length: {}",response->type(),response->code(), response->session_id(), response->session_data().length());
...
ClientPtr tlsSessionCacheClient(Server::Configuration::TransportSocketFactoryContext& factory_context, const envoy::config::core::v3::GrpcService& grpc_service,
  ...
  auto client_or_error = factory_context.clusterManager().grpcAsyncClientManager().getOrCreateRawAsyncClient(grpc_service, factory_context.statsScope(), true);
