...
const ServerContextConfigImpl& config_impl = dynamic_cast<const ServerContextConfigImpl&>(config);
    tls_session_cache_client_ = config_impl.getTlsSessionCacheClient();
    if (tls_session_cache_client_ != nullptr) {
      ...
      * There is no known limit on the size of the created ASN1 representation, so the necessary amount of space should be obtained by first calling i2d_SSL_SESSION() with pp=NULL,
      * and obtain the size needed, then allocate the memory and call i2d_SSL_SESSION() again. Note that this will advance the value contained in *pp so it is necessary to save a copy of the original allocation.
      * For example: int i,j; char *p, *temp; i = i2d_SSL_SESSION(sess, NULL); p = temp = malloc(i); j = i2d_SSL_SESSION(sess, &temp); assert(i == j); assert(p+i == temp);
      */
      ...
      ENVOY_LOG(info, "new ssl session: id:{}, session data len: {}", session_id, session_data_len);
      ...
      server_context_impl->tls_session_cache_client_->storeTlsSessionCache(callbacks, ssl, sslSessionCacheIndex(), session_id,  session_data, session_data_len); 
      ...
      SSL_CTX_sess_set_get_cb(
        ...
        [](SSL* ssl, const uint8_t* id, int id_len, int* out_copy) -> SSL_SESSION* {
          ...
          auto already_request_external = static_cast<Network::TransportSocketCallbacks*>(SSL_get_ex_data(ssl, sslSessionCacheIndex()));
          ENVOY_LOG(info, "missing ssl session: id:{} from internal cache", session_id);
          ENVOY_LOG(debug, "socket index:{}, session cache index:{}", sslSocketIndex(), sslSessionCacheIndex());
          if (already_request_external == nullptr) {
            // TBD: add a metrics to track the internal cache hit rate
            ContextImpl* context_impl =
                static_cast<ContextImpl*>(SSL_CTX_get_app_data(SSL_get_SSL_CTX(ssl)));
            ServerContextImpl* server_context_impl = dynamic_cast<ServerContextImpl*>(context_impl);
            server_context_impl->tls_session_cache_client_->fetchTlsSessionCache(callbacks, ssl, sslSessionCacheIndex(), session_id, session_data, &len);
