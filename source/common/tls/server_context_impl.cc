...
ServerContextImpl::ServerContextImpl(Stats::Scope& scope,
  ...
  for (uint32_t i = 0; i < tls_certificates.size(); ++i) {
    ...
    const ServerContextConfigImpl& config_impl =
        dynamic_cast<const ServerContextConfigImpl&>(config);
    ...
    if (tls_session_cache_client_ != nullptr) {
      ...
      SSL_CTX_sess_set_new_cb(ctx.ssl_ctx_.get(), [](SSL* ssl, SSL_SESSION* session) -> int {
         ...
         * to hold the binary representation of the session. There is no known limit on the size of
         * the created ASN1 representation, so the necessary amount of space should be obtained by
         * first calling i2d_SSL_SESSION() with pp=NULL, and obtain the size needed, then allocate
         * the memory and call i2d_SSL_SESSION() again. Note that this will advance the value
         * contained in *pp so it is necessary to save a copy of the original allocation. For
         * example: int i,j; char *p, *temp; i = i2d_SSL_SESSION(sess, NULL); p = temp = malloc(i);
         * j = i2d_SSL_SESSION(sess, &temp); assert(i == j); assert(p+i == temp);
         */
